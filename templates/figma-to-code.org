...
3. Create implementation plan in the plans directory (check `plans_dir` in `~/.config/opencode/workflows.json`, default: `~/.config/opencode/plans/`). Filename: {DATE}-{SLUG}.org. Include:
