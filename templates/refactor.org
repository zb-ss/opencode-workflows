...
## Requirements
1. Save the plan to the plans directory (check `plans_dir` in `~/.config/opencode/workflows.json`, default: `~/.config/opencode/plans/`). Filename: {DATE}-{SLUG}-refactor.org
