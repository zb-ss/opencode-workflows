...
export class DrainingQueue<T, R> {
  ...
  enqueue(item: T, key: string, run: (item: T) => Promise<R>): Promise<R> {
    return new Promise<R>((resolve, reject) => {
      this.pending.push({ item, key, run, resolve, reject })
      this.drain()
    })
  }
  ...
  private drain(): void {
    ...
    while (started) {
      ...
      void entry.run(entry.item).then(entry.resolve, entry.reject).finally(() => {
